...
// const deleteFile = async (req, res) => {
//   try {
//     const file = await File.findById(req.params.id); // <-- declare first

//     if (!file) {
//       return res.status(404).json({ message: "File not found" });
//     }

//     // ...rest of your delete logic (S3 delete, DB delete, etc)...
//     await file.remove();
...
const deleteFile = async (req, res) => {
  try {
    ...
    // Update user stats
const update = { $inc: { totalUploads: -1 } };
if (file.type.startsWith('image/')) update.$inc.imageCount = -1;
else if (file.type.startsWith('video/')) update.$inc.videoCount = -1;
else if (file.type.startsWith('application/')) update.$inc.documentCount = -1;

await User.findByIdAndUpdate(file.createdBy, update);
